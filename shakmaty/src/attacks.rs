...
mod hyperbola_quintessence {
    ...
    impl Prepared {
        ...
        const fn new_bishop(sq: i32) -> Prepared {
            Prepared {
        ...
        const fn new_rook(sq: i32) -> Prepared {
            Prepared {
    ...
    static PREPARED_BISHOP: [Prepared; 64] = {
        let mut table = [const { Prepared::empty() }; 64];
        let mut sq = 0;
        while sq < 64 {
            table[sq] = Prepared::new_bishop(sq as i32);
            sq += 1;
        }
        table
    };

    static PREPARED_ROOK: [Prepared; 64] = {
        let mut table = [const { Prepared::empty() }; 64];
        let mut sq = 0;
        while sq < 64 {
            table[sq] = Prepared::new_rook(sq as i32);
            sq += 1;
        }
        table
    };
...
pub const fn queen_attacks(sq: Square, occupied: Bitboard) -> Bitboard {
    rook_attacks(sq, occupied).with_const(bishop_attacks(sq, occupied))
}
